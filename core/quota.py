"""
Per-account, per-game quota tracker (mirror of UbiTokeer/core/quota.py).

Key = "<account_name>:<content_id>". Each EA account can mint MAX_TOKENS_PER_DAY
tokens per game per rolling 24h window (EA enforces server-side too via
CG_LIMIT_EXCEEDED; this is our client-side view that drives account rotation).
"""
...
class QuotaTracker:
    ...
    def _key(self, account_name: str, content_id: str) -> str:
        return f"{account_name}:{content_id}"
    ...
    def get_remaining(self, account_name: str, content_id: str) -> int:
        with self._lock:
            key = self._key(account_name, content_id)
    ...
    def can_generate(self, account_name: str, content_id: str) -> bool:
        return self.get_remaining(account_name, content_id) > 0

    def decrement(self, account_name: str, content_id: str) -> None:
        with self._lock:
            key = self._key(account_name, content_id)
            entry = self._data.get(key)
    ...
    def record(self, account_name: str, content_id: str) -> None:
        with self._lock:
            key = self._key(account_name, content_id)
            entry = self._data.get(key)
            now = time.time()
            ...
            logger.info(
                f"Quota recorded: {account_name}:{content_id} — "
                f"{self._data[key]['count']}/{self._daily_limit} used, {remaining} remaining"
            )
            self._save()

    def get_simple(self, content_id: str, accounts: list[dict]) -> dict:
        ...
        for acc in accounts:
            if not acc.get("track_quota", True):
                has_untracked = True
                continue
            remaining = self.get_remaining(acc["name"], content_id)
            total_remaining += remaining
            if remaining == 0:
                key = self._key(acc["name"], content_id)
                entry = self._data.get(key)
                if entry:
        ...
        return {
            ...
            "game_name": game_names.get(content_id, f"EA {content_id}"),
            "remaining": total_remaining,
            "total_remaining": total_remaining,
            "total": self._daily_limit * max(1, len([a for a in accounts if a.get("track_quota", True)])),
            "next_slot_in_ms": next_slot_ms,
            "resets_in": resets_in,
        }

    def get_summary(self, accounts: list[dict]) -> dict:
        """{content_id: {total_remaining, total, ...}} for the bot's stock panel."""
        game_names = load_game_names()
        now = time.time()
        result = {}
        ...
        for acc in accounts:
            for cid in acc.get("content_ids", []):
                content_map.setdefault(cid, []).append(acc)

        with self._lock:
            for content_id, accs in content_map.items():
                acc_details = []
                total_used = 0
                for acc in accs:
                    ...
                    if not track:
                        acc_details.append({
                            "name": acc["name"], "used": -1, "remaining": -1,
                            "resets_in": None, "track_quota": False,
                        })
                        continue
                    ...
                    total_used += used
                    acc_details.append({
                        "name": acc["name"], "used": used, "remaining": remaining,
                        "resets_in": resets_in, "track_quota": True,
                    })
